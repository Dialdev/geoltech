...
(function() {
  ...
  __wm.bt = function bootstrap(
    ...
    function showTrackers(event) {
      ...
      if (val) {
        ...
        $ipp.className = "";
        $y.innerHTML = displayYear;
        $m.innerHTML = prettyMonths[displayMonth - 1];
        $d.innerHTML = displayDay;
      }
    ...
    function getElementX2(el) {
      ...
      var box =
        typeof el.getBoundingClientRect !== "undefied"
          ? el.getBoundingClientRect()
          : { top: 0, left: 0 };
      return (
        box.left + (window.pageXOffset || de.scrollLeft) - (de.clientLeft || 0)
      );
    }
    function navCaptures(captures) {
      ...
      var first_ts = captures.first_ts,
        last_ts = captures.last_ts;
    ...
    new wbAutoComplete({
      ...
      source: function(query, suggest) {
        ajax(
          "GET",
          "/__wb/search/host?q=" + encodeURIComponent(query),
          function(data) {
            ...
            if (typeof data.hosts !== "undefined" && data.hosts.length > 0) {
              ...
              ajax(
                "GET",
                "/__wb/search/anchor?q=" + encodeURIComponent(query),
                function(data) {
                  var data = _JSON.parse(data.response);
  ...
  function show_timestamps() {
    ...
    function find_elements_by_url(current_window, url) {
      ...
      for (var i = 0; i < current_window.frames.length; i++) {
        try {
          var frame_els_array = find_elements_by_url(
            current_window.frames[i].window,
            url
          );
    ...
    function find_elements_by_tag_name(current_window, tag) {
      ...
      for (var i = 0; i < current_window.frames.length; i++) {
        try {
          var frame_els_array = find_elements_by_tag_name(
            current_window.frames[i].window,
            tag
          );
          els_array = els_array.concat(frame_els_array);
    ...
    for (var i = 0, len = imgs.length; i < len; i++) {
      ...
      if (
        !imgs[i].src ||
        imgs[i].src.startsWith(static_prefix) ||
    ...
    for (i = 0, len = scripts.length; i < len; i++) {
      if (
        !scripts[i].src ||
        scripts[i].src.startsWith(static_prefix) ||
    ...
    for (i = 0, len = links.length; i < len; i++) {
      if (
        !links[i].href ||
        links[i].href.startsWith(static_prefix) ||
  ...
  function userIsNotLoggedIn() {
    hide(["wm-save-snapshot-open", "wm-save-snapshot-in-progress"]);
    show("wm-sign-in");
  }
  ...
  function startSnapShotSaving() {
    ...
    show(["wm-save-snapshot-in-progress"]);
  }

  function successSnapshotSaving() {
    hide(["wm-save-snapshot-fail", "wm-save-snapshot-in-progress"]);
    show(["wm-save-snapshot-open", "wm-save-snapshot-success"]);
  }

  function failSnapshotSaving(err) {
    hide(["wm-save-snapshot-in-progress", "wm-save-snapshot-success"]);
    show(["wm-save-snapshot-fail", "wm-save-snapshot-open"]);
  }
